...
def display_keypoints_instances(image, boxes, masks, keypoints, class_ids, class_names,
                      skeleton = [], scores=None, title="",
                      figsize=(16, 16), ax=None):
    """
    ...
    scores: (optional) confidence scores for each box
    figsize: (optional) the size of the image.
    ### Written by: X. Du -- adding display_keypoints and display_instances together.
    """
    # Number of persons
    N = boxes.shape[0]
    ...
    masked_image = skeleton_image.astype(np.uint32).copy()  # changed from "image" to "skeleton_image"
    for i in range(N):
        color = colors[i]
        ...
        for verts in contours:
            ...
            ax.add_patch(p)
    ax.imshow(masked_image.astype(np.uint8))
    #ax.imshow((skeleton_image.astype(np.uint8) + masked_image.astype(np.uint8)))
